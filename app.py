...
class ForexCertaintySystem:
    ...
    def check_retail_pattern(self):
        ...
        if abs(self.today_data['retail_distance_pips']) > 10:
            pattern = {
                ...
                'conditions_met': [
                    ...
                    f"Retail avg: {self.today_data['retail_long_avg'] if self.today_data['retail_net_position'] == 'LONG' else self.today_data['retail_short_avg']}"
                ]
            }
            return pattern
        return None
    ...
    def check_holiday_pattern(self):
        ...
        if self.today_data['holiday_type'] != 'NONE' and self.today_data['days_to_next_holiday'] <= 2:
            pattern = {
                'id': 'HOLIDAY_RANGE_95',
                'name': 'Holiday Range Compression',
                'description': 'Days before major holidays exhibit predictable range compression',
                'conditions_met': [
                    f"Days to holiday: {self.today_data['days_to_next_holiday']}",
                    f"Holiday type: {self.today_data['holiday_type']}",
                    f"Expected liquidity: {self.today_data['expected_liquidity']}",
                    f"Consolidation score: {self.today_data['consolidation_score']}"
                ]
            }
            return pattern
        return None
        ...
        if 13 <= current_hour <= 16:
            pattern = {
                ...
                'conditions_met': [
                    ...
                    f"Trend strength: {self.today_data['trend_strength']}",
                    f"Daily direction: {self.today_data['daily_direction']}",
                    f"London range: {self.today_data['london_session_range']} pips"
                ]
            }
            return pattern
        return None
    ...
    def check_gap_pattern(self):
        ...
        gap_size = abs(self.today_data['weekly_open'] - self.today_data['daily_open']) * 10000
        if gap_size > 25:
            pattern = {
                'id': 'GAP_FILL_92',
                ...
                'conditions_met': [
                    ...
                    f"Weekly open: {self.today_data['weekly_open']}",
                    f"Daily open: {self.today_data['daily_open']}",
                    f"Current price: {self.today_data['current_price']}"
                ]
            }
            return pattern
    ...
    def _generate_retail_trade(self, pattern, confidence):
        ...
        if self.today_data['retail_net_position'] == 'LONG':
            direction = 'SELL'
            entry_price = self.today_data['retail_long_avg'] - 0.0002
            stop_loss = entry_price + 0.0017
            take_profit_1 = entry_price - 0.0015
            take_profit_2 = entry_price - 0.0028
        else:
            direction = 'BUY'
            entry_price = self.today_data['retail_short_avg'] + 0.0002
            stop_loss = entry_price - 0.0017
            take_profit_1 = entry_price + 0.0015
            take_profit_2 = entry_price + 0.0028
    ...
    def _generate_news_trade(self, pattern, confidence):
        ...
        current_price = self.today_data['current_price']
    ...
    def _generate_holiday_trade(self, pattern, confidence):
        ...
        current_price = self.today_data['current_price']
        predicted_range = self.today_data['daily_range_pips'] * 0.5  # 50% compression
    ...
    def _generate_overlap_trade(self, pattern, confidence):
        ...
        direction = self.today_data['daily_direction']
        current_price = self.today_data['current_price']
    ...
    def _generate_gap_trade(self, pattern, confidence):
        ...
        gap_direction = 'UP' if self.today_data['daily_open'] > self.today_data['weekly_open'] else 'DOWN'
        gap_size = abs(self.today_data['weekly_open'] - self.today_data['daily_open']) * 10000
        
        if gap_direction == 'UP':
            direction = 'SELL'  # Fade the gap up
            entry_price = self.today_data['daily_open'] + 0.0005
            stop_loss = entry_price + gap_size * 1.5 * 0.0001
            take_profit = self.today_data['weekly_open']
        else:
    ...
    def get_market_metrics(self):
        ...
        return {
            'volatility_score': self.today_data['volatility_score'],
            'trend_strength': self.today_data['trend_strength'],
            'consolidation_score': self.today_data['consolidation_score'],
            'daily_range': self.today_data['daily_range_pips'],
            'weekly_range': self.today_data['weekly_range_pips'],
            'retail_bias': self.today_data['retail_net_position'],
            'news_impact': 'HIGH' if self.today_data['high_impact_news_count'] > 0 else 'LOW',
            'liquidity_status': self.today_data['expected_liquidity']
        }
    ...
    def get_historical_performance(self, days=30):
        ...
        # Simulate pattern occurrences (in real implementation, you'd track actual trades)
        pattern_counts = {
...
def main():
    ...
    try:
        ...
        with col1:
            st.metric("Current Price", f"{system.today_data['current_price']:.5f}", 
                     f"{system.today_data['daily_change_pct']:.2f}%")
        with col2:
            st.metric("Daily Range", f"{system.today_data['daily_range_pips']} pips", 
                     system.today_data['daily_direction'])
        with col3:
            st.metric("Weekly Direction", system.today_data['weekly_direction'], 
                     f"{system.today_data['weekly_range_pips']} pips")
        with col4:
            news_count = system.today_data['high_impact_news_count']
            st.metric("High Impact News", news_count, 
                     f"Next: {system.today_data['next_major_news_time']}" if news_count > 0 else "None")
        ...
        with mcol1:
            ...
            <div class='metric-card'>
                <h4>Volatility</h4>
                <h2>{metrics['volatility_score']:.2f}</h2>
                <p>{'High' if metrics['volatility_score'] > 0.7 else 'Medium' if metrics['volatility_score'] > 0.4 else 'Low'}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with mcol2:
            ...
            <div class='metric-card'>
                ...
                <h2>{metrics['trend_strength']:.2f}</h2>
                <p>{'Strong' if metrics['trend_strength'] > 0.6 else 'Weak' if metrics['trend_strength'] < 0.4 else 'Moderate'}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with mcol3:
            ...
            <div class='metric-card'>
                <h4>Consolidation</h4>
                <h2>{metrics['consolidation_score']:.2f}</h2>
                <p>{'Range-bound' if metrics['consolidation_score'] > 0.6 else 'Trending'}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with mcol4:
            ...
            <div class='metric-card'>
                <h4>Retail Bias</h4>
                <h2>{metrics['retail_bias']}</h2>
                <p>{system.today_data['retail_distance_pips']} pips from avg</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with mcol5:
            ...
            <div class='metric-card'>
                <h4>Liquidity</h4>
                <h2>{metrics['liquidity_status']}</h2>
                <p>{'Normal' if metrics['liquidity_status'] == 'NORMAL' else 'Reduced'}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        if patterns:
            ...
            for i, trade in enumerate(trades):
                ...
                with st.container():
                    ...
                    with col3:
                        st.metric("Risk/Reward", f"1:{trade.get('risk_reward_ratio', 'N/A')}")
        ...
        st.info("Make sure the CSV is available at the specified URL.")
